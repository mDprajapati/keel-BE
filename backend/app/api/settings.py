...
@router.get("/settings", response_model=ApiResponse[WorkspaceSettings])
async def get_settings(
    principal: Principal = Depends(get_current_user), db: AsyncSession = Depends(get_db)
):
    return ok(await settings_service.get_settings(db, workspace_id=principal.workspace_id))


@router.patch("/settings", response_model=ApiResponse[WorkspaceSettings])
